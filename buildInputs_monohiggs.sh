...
if [ ${data} = "zpbaryonic" ]; then
    ...
    if [ ${year} -eq 2016 ]; then
        cp ${BASEDIR}/hadronizer/monoHiggsToBB_zpbaryonic/hadronizer_${dname}.py ./submit/input/
...
if [ ${data} = "2hdma" ]; then
    ...
    echo "You're producing mono Higgs sample with 2HDM+a model!"
    ...
    if [ ${year} -eq 2016 ]; then
        cp ${BASEDIR}/hadronizer/monoHiggsToBB_2hdma/hadronizer_${dname}.py ./submit/input/
        cp inputs/aod_template2016.py ./submit/input/
        cp inputs/pu_files2016.py ./submit/input/
        cp inputs/mc_NANO_2016.py ./submit/input/
        cp ${BASEDIR}/exec2016.sh $SUBMIT_WORKDIR
